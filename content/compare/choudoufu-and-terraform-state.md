...
Twenty of them, each a smoke scenario of a few minutes. Each runs inverted under `BREAK=1`, so a check that cannot fail is caught. Staleness costs reads and never results. Recovery is a re-run rather than surgery. The tag is the boundary, enforced by AWS rather than by the tool. [Run them yourself.](https://intentius.io/choudoufu/docs/claims/)

## Where it stops

AWS only, and experimental. About half the provider's types carry no tags, so they are identifiable but not governable by a tag condition. Identity must be knowable before create, and `live-check` tells you what refuses with no cloud call.
